...
mibd_meta_pacl2=mibd_meta_patho%>%
...
mibd_meta=mibd_meta_raw%>%
  mutate(ROILabel=as.numeric(ROILabel))%>%
  left_join(mibd_meta_pacl2, by=c("ROILabel"="TMA 순번","set"="set"))
...
```{r}
ds2=CreateSeuratObject(data_norm_2)
ds2@assays$RNA$data=ds2@assays$RNA$counts

SP$barcode=rownames(SP)

#rownames(sp)=sp$SegmentDisplayName
SP=SP[SP$SegmentDisplayName%in%rownames(ds2@meta.data),]

ds2=AddMetaData(ds2, SP)
...
# 다시 Seurat 객체에 덮어쓰기
ds2@meta.data$heal <- md$heal

ds2@meta.data$response=case_when(
  ds2@meta.data$heal%in%c(0,1)~"NR",
  ds2@meta.data$heal%in%c(2,3,4)~"R",
  TRUE ~ "NA"
)
```
...
data_str_post_ved=subset(data_str_post, 약제종류=="Vedolizumab")
...
### by pre vs post
```{r}
m.pre=FindMarkers(data_seurat, ident.1="pre", group.by="treat")%>%marker_filter()%>%.[.$p_val<0.05,]%>%.[.$avg_log2FC>2,]
m.ck.pre=FindMarkers(data_ck, ident.1="pre", group.by="treat" )%>%marker_filter()%>%.[.$p_val<0.05,]%>%.[.$avg_log2FC>2,]
m.str.pre=FindMarkers(data_str, ident.1="pre", group.by="treat" )%>%marker_filter()%>%.[.$p_val<0.05,]%>%.[.$avg_log2FC>2,]

m.pre.mi=FindMarkers(data_seurat, ident.1="pre", group.by="treat")%>%marker_filter()%>%.[.$p_val<0.05,]%>%.[.$avg_log2FC< -2,]
m.ck.pre.mi=FindMarkers(data_ck, ident.1="pre", group.by="treat" )%>%marker_filter()%>%.[.$p_val<0.05,]%>%.[.$avg_log2FC< -2,]
m.str.pre.mi=FindMarkers(data_str, ident.1="pre", group.by="treat" )%>%marker_filter()%>%.[.$p_val<0.05,]%>%.[.$avg_log2FC< -2,]


paste(m.pre$gene[1:100], collapse = ", ")
paste(m.pre.mi$gene[1:100], collapse = ", ")
paste(m.ck.pre$gene[1:100], collapse = ", ")
paste(m.ck.pre.mi$gene[1:100], collapse = ", ")
paste(m.str.pre$gene[1:100], collapse = ", ")
paste(m.str.pre.mi$gene[1:100], collapse = ", ")
