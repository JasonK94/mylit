...
mibd_meta_clinical=read_xlsx("/data/kjc1/projects/#124. mIBD/1. biologics naive 이름_병록번호 제외_trim.xlsx")%>%
  .[!names(.)%in%c("treatment", "slide","TMA 순번")]
mibd_meta_pacl=mibd_meta_patho%>%left_join(mibd_meta_clinical, by="병리번호")
mibd_meta=mibd_meta_raw%>%mutate(ROILabel=as.numeric(ROILabel))%>%left_join(mibd_meta_pacl, by=c("ROILabel"="TMA 순번","set"="set"))
raw_data=mibd
SP=mibd_meta
sp=SP%>%
  .[.$treat=="pre",]%>%
...
SP=subset(SP,SegmentDisplayName!=remove_aoi)
...
md <- md %>% mutate(
  heal = case_when(
    ...
    `histologic Endoscopic improvement` == 1          ~ 3,
    `Endo remission` == 1                             ~ 2,
    `mucosal healing` == 1                            ~ 1,
    `mucosal healing` == 0                            ~ 0,
