...
### 1.5 FGS v5.4 생성 및 IS6(`is5s`) 테스트

- `FGS()` 기본 호출이 `find_gene_signature_v5.4()`를 타도록 변경하여 ranger/glmnet/NMF 개선 경로(`method_impl = "v5.4"`)를 항상 사용
- `find_gene_signature_v5.4()`는 v5.3 API를 그대로 따르되 내부적으로 `method_impl = "v5.4"`를 전달
- `TML7` + `compute_meta_gene_importance` + `add_meta_signature_score` 조합으로 `/data/user3/sobj/IS6_sex_added_0.1x_251110.qs` 데이터를 사용해 end-to-end 검증
- 실행 절차:
  1. `cd /home/user3/data_user3/git_repo/_wt/fgs/st`
  2. `R -q` 진입 후 `source('start.R')`로 renv 초기화
  3. `source('../myR/R/signature.R')`로 최신 함수 재로딩
  4. `qs::qread()`로 is5s 불러오기 → `find_gene_signature_v5.4()` → `TML7()` → `compute_meta_gene_importance()` → `add_meta_signature_score()`
  5. `predict(tml$best_model, type = "prob")[, positive_class]` 사용, `pred_logit = log(pred/(1-pred))`
  6. `AddMetaData()`로 `pred`, `pred_logit` 저장 후 `pROC::roc()`로 meta score vs TML logit AUC 비교
- 산출물(`fgs.each.is5s.qs`, `tml.each.is5s.qs`)은 `outputs/fgs_is5s/`에 저장
