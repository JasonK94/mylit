...
CONFIG="config/manifest_hto_fixed.csv"
RUN_ID="run_hto_v1"
# No downsampling for full run

echo "Starting FULL pipeline run with Run ID: $RUN_ID"

echo "Running Step 1: Read and Demultiplex"
Rscript scripts/pipe/pipe1_read_demulti.R --config $CONFIG --run_id $RUN_ID --input_step 0 --output_step 1
...
echo "FULL Pipeline run completed successfully!"
