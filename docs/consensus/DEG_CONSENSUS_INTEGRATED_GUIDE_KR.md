...
### 1.2 방법론 아키텍처 및 Refactoring (2025)

현재 DEG Consensus 모듈은 과도기적 단계에 있으며, 두 가지 형태의 구현이 공존합니다:

1.  **Phase 1 (Legacy Wrappers)**: `runMUSCAT` 등 `muscat` 패키지를 그대로 래핑한 함수들.
    *   장점: 구현이 쉽고 원본 패키지의 동작을 보장.
    *   단점: Design Matrix 제어, NA 처리, 결과 포맷팅 등 커스터마이징이 제한적.
2.  **Phase 2 (Standalone/Native)**: `runEDGER_LRT_v1`, `runLIMMA_voom_v1` 등 **자체 구현된** 표준화 함수들.
    *   특징: `muscat::aggregateData`로 Pseudobulk 생성 후, 각 방법론(edgeR, DESeq2, limma)의 네이티브 함수를 직접 호출.
    *   **장점**:
        *   **유연성**: 공변량(`covar_effects`)을 포함한 복잡한 Design Matrix(`~ 0 + Group + Batch + Covariates`)를 완벽히 제어.
        *   **안정성**: 엄격한 NA 처리 및 예외 처리 로직 내장.
        *   **확장성**: 향후 모델 변경이 용이.
    *   **계획**: 향후 모든 방법론은 **Phase 2 방식**으로 통합될 예정이며, Muscat 래퍼들은 점진적으로 Deprecate 될 예정입니다.
...
### 3.1 방법론별 변수 지원 및 기능 비교 (Feature Support Matrix)

각 DEG 방법론이 현재 파이프라인에서 지원하는 변수 범위와 모델링 능력입니다.

| 방법론 (Method) | Fixed Effect (Group) | Covariates (Sex, Age, etc.) | Random Effect (Subject) | Count Data Requirement | 비고 |
| :--- | :---: | :---: | :---: | :---: | :--- |
| **edgeR-LRT/QLF** | ✅ | ✅ (Multiple) | ❌ | Raw Counts | Pseudobulk 후 사용. GLM 기반. |
| **DESeq2-Wald/LRT** | ✅ | ✅ (Multiple) | ❌ | Integers (Automatically Rounded) | Pseudobulk 후 사용. GLM 기반. |
| **limma-voom** | ✅ | ✅ (Multiple) | ❌ | Raw Counts | Pseudobulk 후 사용. Linear Model. |
| **limma-trend** | ✅ | ✅ (Multiple) | ❌ | LogCPM or Counts | Pseudobulk 후 사용. |
| **muscat (wrappers)**| ✅ | ✅ (Partial) | ❌ | Raw Counts | `muscat` 내부 로직에 의존. customizing 제한적. |
| **NEBULA** | ✅ | ✅ | ✅ (Subject) | Raw Counts | **Single-cell 수준 분석**. Mixed Model (NBLMM). |
| **dream** (Planned) | ✅ | ✅ | ✅ (Subject) | Raw Counts | Pseudobulk 수준 Mixed Model. |

*   **Fixed Effect**: 주요 비교 변수 (예: `g3` 그룹).
*   **Covariates**: 보정하고 싶은 공변량 (성별, 나이, 등). 현재 파이프라인에서 `covar_effects` 인자로 전달 시 모든 Pseudobulk 방법론(edgeR, DESeq2, limma)의 Design Matrix에 포함됩니다.
*   **Random Effect**: 개체(Patient) 간 변이를 확률 변수로 모델링. 현재 **NEBULA**만 지원하며, Pseudobulk 방법론들은 샘플 단위로 집계(Aggregate)함으로써 개체 변이를 암묵적으로 해결하는 Fixed Effect Model을 사용합니다.

### 3.2 모델링 가이드: 변수 보정에 대한 과학적 고찰

**Q: 그룹 간 차이가 유의하지 않은 변수(예: 성별, 나이)도 보정해야 할까요?**

scRNA-seq 및 의과학 연구 맥락에서 다음과 같이 권장합니다:

1.  **의과학 논문 통상적 수준 (Standard Practice)**:
    *   **권장**: **성별(Sex), 나이(Age), 배치(Batch)** 등 생물학적으로 유전자 발현에 영향을 줄 것으로 알려진 주요 변수는 그룹 간 차이(p-value) 유무와 관계없이 보정하는 것이 통례입니다 ("Table 1" 변수들).
    *   **이유**: 이들 변수는 교란 변수(Confounder)로 작용할 수 있을 뿐만 아니라, 종속 변수(유전자 발현)의 설명되지 않은 변동(Residual Variance)을 줄여주어 주 관심 변수(`g3`)의 검정력(Power)을 높여줍니다.

2.  **통계적 합당성 (Statistical Validity)**:
    *   **주의**: 표본 수(Sample Size)에 비해 너무 많은 공변량을 넣으면 자유도(Degree of Freedom)가 부족해져 오히려 검정력이 떨어질 수 있습니다.
    *   **가이드**:
        *   샘플 수(N)가 적은 경우 (예: 그룹당 3~5명): 핵심 변수 1~2개(예: Sex or Batch)만 보정하거나 보정 없이 분석.
        *   샘플 수가 충분한 경우 (예: 그룹당 10명 이상): 주요 임상 변수(Sex, Age, HTN, DM 등)를 포함 가능.
        *   **오버피팅 방지**: 변수 간 상관관계가 높은 경우(공선성) 하나만 선택해야 합니다.

3.  **"과학자적" 관점 (nonlinear/interactions)**:
    *   대부분의 DEG 모델(edgeR, DESeq2)은 **선형적(Linear)** 관계를 가정합니다.
    *   비선형적 관계(예: 나이가 들수록 발현량이 U자형으로 변함)나 복잡한 상호작용(Interaction)이 의심될 경우, 단순한 공변량 추가(`+ Age`)로는 해결되지 않습니다.
    *   하지만 1차적 스크리닝인 DEG 분석에서는 선형 보정이 가장 효율적이고 해석 가능한 접근임이 인정됩니다. 복잡한 관계는 별도의 심층 분석(ML/DL 등)으로 다루는 것이 좋습니다.

**결론**: `g3` 그룹 간 차이가 없더라도, **Sex, Age, Batch**는 기본적으로 모형에 포함하여(`covar_effects`) 보정하는 것을 권장합니다 (자유도가 허용하는 한).
