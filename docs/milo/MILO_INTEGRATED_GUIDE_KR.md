...
flowchart TD
    ...
    subgraph Save["8. 결과 저장"]
        ...
        SavePlotData["플롯 입력(가공 데이터) 저장<br/>04_plot_inputs.qs"]
        SavePlotBundle["플롯 묶음 저장<br/>05_plot_bundle.qs"]
        SavePlots["필요시 이미지 출력<br/>.png, .pdf"]
        SaveMilo --> FinalResult[("최종 결과")]
        SaveResults --> FinalResult
        SavePlotData --> FinalResult
        SavePlotBundle --> FinalResult
...
*   `density_plot`: Density Plot 생성 여부 (기본값: FALSE)
*   `density_metric`: Density 가중치 컬럼 (기본값: NULL -> logFC 사용)

#### 반환값
*   `milo`: Milo 객체
*   `da_results`: 차등 풍부도 검정 결과 (logFC 방향 정보 포함)
*   `plot_inputs`: 시각화를 재현하기 위해 준비된 Milo/DA/메트릭 데이터 (`plotting = FALSE`이면 `NULL`)
*   `plots`: 세션 내에서 즉시 사용할 수 있는 ggplot 리스트(그래프/UMAP/beeswarm/조합 플롯)로, 저장 시에는 PNG/PDF 파일로 출력됨

### Density Plot
*   `run_milo_pipeline(..., density_plot = TRUE)`로 활성화할 수 있습니다.
*   공간적인 변화 양상(Spatial Average logFC)을 부호를 보존하여 시각화합니다.
*   `plot_milo_density()` 함수를 독립적으로 사용하여 세부 조정이 가능합니다.
...
### 캐시 파일 구조
*   `01_nhoods_built.qs` / `02_distances_calculated.qs`: 그래프 및 거리 캐시
*   `03_tested.qs` / `03_da_results.qs`: DA 분석 결과 (milo 객체 + DA 테이블)
*   `04_plot_inputs.qs`: 시각화를 재현하기 위해 준비된 Milo/DA/메트릭 데이터 (플롯 입력 전용)
*   `05_plot_bundle_*.(png|pdf)`: ggplot 결과물을 각각 PNG/PDF로 저장한 묶음(그래프, beeswarm, UMAP, 조합 플롯 등)
*   필요 시 추가 형식의 이미지 출력은 별도 스크립트에서 생성
