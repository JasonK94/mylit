...
| `density_plot` | Density plot 생성 여부 | `FALSE` |
| `density_metric` | Density plot 가중치 (기본: "logFC") | `NULL` |

#### 반환값
- `milo`: Milo 객체
- `da_results`: 차등 풍부도 검정 결과 (logFC 방향, 비교 정보 포함)
- `plot_inputs`: 시각화를 재현하기 위한 Milo/DA/메트릭 데이터 (`plotting = FALSE`이면 `NULL`)
- `plots`: 세션 내에서 즉시 사용 가능한 ggplot 리스트(그래프/UMAP/beeswarm/조합 플롯). 파일로 저장할 때는 PNG/PDF로 출력됨.

### Density Plot (신규 기능)
- **Weighted Density**: `density_metric` (예: `logFC`)을 가중치로 사용하여 2D density를 그립니다.
- **Spatial Average**: `density_metric="logFC"`를 사용하면, 각 지역의 평균 logFC를 계산하여 Up/Down-regulation 영역을 시각화합니다 (`plot_milo_density(mode="average")`로 내부 호출).
- 파이프라인에서 `density_plot = TRUE`로 설정하면 자동으로 생성되어 `_05` 번들에 포함됩니다.
...
### 캐시 파일 구조 안내
- `01_nhoods_built.qs` / `02_distances_calculated.qs`: 그래프 및 거리 단계 캐시
- `03_tested.qs` / `03_da_results.qs`: Milo 객체와 DA 결과
- `04_plot_inputs.qs`: Plot-ready Milo/DA/metric data (used to re-render figures)
- `05_plot_bundle_*.(png|pdf)`: Exported ggplot outputs (graph, beeswarm, UMAP, combined panels)
...
- Plot bundle 캐시는 이미지(`*.png`, `*.pdf`) 형태로 저장되며, `cache_files$plot_bundle`를 통해 출력 경로를 제어할 수 있습니다.
