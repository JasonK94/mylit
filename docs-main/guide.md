...
- **mIBD scRNAseq data**: (name TBD), /data/user3/sobj/IBD_biologics.rds

### 3. Variable Descriptions

1) Configure in `scripts/vars_config.R` file and source it to reduce errors.

2) **stroke**
`g3` is the target variable ("2", "1", "NA"; note that NA is a string. Be careful not to mistake it as numeric when removing NA), patients are in `$hos_no` (8-digit number; be careful not to mistake as numeric), cluster information is in `$anno3.scvi` (approximately 22 types). `$GEM` and `$set` are random effects other than patients, but since there are 23 patients, `$GEM` has a maximum of 12, and `$set` has a maximum of 3, they can be treated as fixed effects.
...
3) **mIBD GeoMx**
`$emrid` is the patient, with 114 samples (much fewer). With `$drug` (3 drugs), `$ck` (tissue type; PanCK positive or negative; TRUE, FALSE), `$treatment` ("pre", "post"), `$response` ("R", "NR"), there are many analysis target variables and considerations, and the data is small, making it more challenging.
...
2) Scripts for each worktree should be stored and managed in `scripts/<wt_name>`, logs in `logs/<wt_name>`, and docs in `docs/<wt_name>`.
3) Scripts, logs, and docs that have expired purposes (mainly test scripts) should be deleted or moved to a subfolder `archive/`.
4) This is to prevent conflicts when merging each branch into main or merger branch.
5) It is good to keep the root folder clean: maintain only `guide.md`, `.gitignore`, `.Rprofile`, etc.

### 5. Objectives
1) **Statistically rigorous yet powerful analysis (DEG analysis) tool development**: Mainly in myR/analysis.R, being developed in the analysis branch, `_wt/analysis` worktree. Previously the main2 branch/worktree. Expected to be integrated into deg-consensus.
