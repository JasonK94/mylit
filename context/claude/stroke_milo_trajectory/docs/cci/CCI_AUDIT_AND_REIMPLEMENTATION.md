...
### MNN Permissive 재실행: ❌ 근본적 한계 확인
- anno2 + 최대한 완화된 thresholds로도 `group_prioritization_tbl` = **0 rows**
- **근본 원인**: `lr_target_prior_cor` step에서 cell type × condition당 sample ≥5 필요 → 불충족
- **결론**: multinichenetr는 이 데이터 구조와 **근본적으로 비호환**
- 기존 intermediate tables (DE genes, ligand activities)는 여전히 활용 가능
- **향후**: LIANA 등 대체 CCI method 검토 필요
...
| Contrast specification | ✅ OK | "Stroke-HC" (L1), "X2-X1" (L2) |
| Cell type filtering | ✅ OK | 18-19/21 cell types retained (pDC, Platelet.PLA excluded) |
| DE analysis | ✅ OK | 78K genes (L1), 70K (L2) |
| Ligand activity | ✅ OK | 1.2M (L1), 525K (L2) pairs |
| **lr_target_prior_cor** | ❌ NULL | "For no celltypes, sufficient samples (>=5) available" |
| **group_prioritization_tbl** | ❌ 0 rows | 핵심 출력 테이블 비어있음 |

**핵심 문제**: Correlation step 실패 → prioritization 0 rows. 이는 MNN의 `generate_prioritization_tables()`가 `lr_target_prior_cor`에 의존하기 때문. 근본 원인은 pseudobulk sample 수 부족 (cell type × condition 조합에서 5개 미만 샘플).
...
**문제**: `lr_target_prior_cor`가 NULL → `generate_prioritization_tables()`에서 correlation score를 포함한 최종 prioritization 불가.

**근본 원인**: multinichenetr의 correlation 단계는 sender-receiver 쌍별로 최소 5개 sample이 필요. anno1 (19 cell types) × 2 conditions에서 sparse cell types (cDC1, ISG+ T_cell 등)는 condition당 2-3 sample에서만 충분한 cell 수를 가짐.

**추가 문제**: `group_prioritization_tbl`이 0 rows인 것은 correlation만의 문제가 아닐 수 있음. `sender_receiver_de` 테이블이 제대로 생성되었는지 확인 필요.
...
2. **MNN prioritized interactions**: ❌ (존재하지 않음)
   - `group_prioritization_tbl` = 0 rows
