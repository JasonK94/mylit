...
- **Details**:
  ...
  - Partial cache overrides now force unspecified stages to recompute and automatically append a timestamp-based suffix (when `save = TRUE` & `suffix = NULL`) so new outputs never clobber existing caches.
  - Fixed `test_cluster_logfc_bias()` block permutation to shuffle block-level means (weighted by neighborhood counts) instead of shuffling within blocks, eliminating the pathological two-valued `p_perm`.
