# ==============================================================================
# Pseudobulk Analysis Functions - Refactored and Modularized
# ==============================================================================
#
# This file contains refactored pseudobulk analysis functions with a clear
# naming convention:
...
#'
#' @param obj Seurat object or data frame with metadata
#' @param required_cols Character vector of required column names
#' @param metadata_name Optional name for the metadata (for error messages)
#'
#' @return NULL (stops with error if validation fails)
#' @keywords internal
pb_utils_validate_metadata <- function(obj, required_cols, metadata_name = "metadata") {
  if (inherits(obj, "Seurat")) {
    meta_cols <- colnames(obj@meta.data)
  } else if (is.data.frame(obj)) {
    meta_cols <- colnames(obj)
  } else {
    stop("obj must be either a Seurat object or a data frame")
  }

  missing_cols <- required_cols[!required_cols %in% meta_cols]
  if (length(missing_cols) > 0) {
    stop("다음 컬럼이 ", metadata_name, "에 없습니다: ",
...
#' @param min_samples Minimum required samples per group
#' @param verbose Logical, whether to print messages
#'
#' @return Logical vector indicating which groups have sufficient samples
#' @keywords internal
pb_utils_check_samples <- function(meta, group_col, min_samples = 2, verbose = TRUE) {
  if (!requireNamespace("dplyr", quietly = TRUE)) {
    stop("dplyr 패키지가 필요합니다.")
  }
  ...
  sufficient <- sample_counts$n >= min_samples

  if (verbose && any(!sufficient)) {
    insufficient_groups <- sample_counts[[group_col]][!sufficient]
    message("경고: 다음 그룹은 최소 샘플 수(", min_samples, ")를 만족하지 못합니다: ",
            paste(insufficient_groups, collapse = ", "))
  }

  stats::setNames(sufficient, sample_counts[[group_col]])
}


#' Build Design Formula
#'
#' @param condition_col Main condition column name
#' @param covariates Optional vector of covariate column names
#' @param custom_formula Optional custom formula (overrides automatic building)
#'
#' @return Formula object
#' @keywords internal
pb_utils_build_design <- function(condition_col, covariates = NULL, custom_formula = NULL) {
  if (!is.null(custom_formula)) {
    if (inherits(custom_formula, "formula")) {
      return(custom_formula)
    } else if (is.character(custom_formula)) {
      return(as.formula(custom_formula))
    } else {
      stop("custom_formula must be either a formula or character string")
    }
  }

  formula_str <- paste("~", condition_col)
...
#' Standardize DEG Results
#'
#' @param results Results data frame from DEG analysis
#' @param method Method used ("edgeR", "DESeq2", "wilcox", "t-test")
#' @param add_gene_col Logical, whether to add gene column from rownames
#'
#' @return Standardized results data frame
#' @keywords internal
pb_utils_format_results <- function(results, method = "edgeR", add_gene_col = TRUE) {
  if (!requireNamespace("dplyr", quietly = TRUE)) {
    stop("dplyr 패키지가 필요합니다.")
  }
  if (!requireNamespace("tibble", quietly = TRUE)) {
    stop("tibble 패키지가 필요합니다.")
  }

  # Add gene column from rownames if requested
  if (add_gene_col && !"gene" %in% colnames(results)) {
    results <- results %>% tibble::rownames_to_column("gene")
  }
  ...
  standard_cols <- c("gene", "avg_log2FC", "p_val", "p_val_adj")
  other_cols <- setdiff(colnames(results), standard_cols)
  col_order <- c(intersect(standard_cols, colnames(results)), other_cols)
...
#' @param target_val Target value for group 1
#' @param labels Character vector of length 2 for group labels
#'
#' @return Character vector of group assignments
#' @keywords internal
pb_utils_create_binary_groups <- function(meta, col, target_val,
                                           labels = c("GROUP1_TARGET", "GROUP2_REFERENCE")) {
  ifelse(meta[[col]] == target_val, labels[1], labels[2])
}
...
#'
#' Prepares pseudobulk expression data from a Seurat object for differential
#' expression analysis using edgeR. Aggregates single-cell data to the sample
#' level and creates necessary data structures including DGEList and design matrix.
#'
...
#' @param cluster_col Metadata column identifying cell clusters/types
#' @param group_col Metadata column identifying experimental groups
#' @param min_count Minimum count threshold for filterByExpr (default: 10)
#' @param norm_method Normalization method for edgeR (default: "TMM")
#' @param design_formula Optional custom design formula
#' @param verbose Print progress messages (default: TRUE)
#'
#' @return List containing:
#'   \item{pb}{Pseudobulk expression matrix}
#'   \item{meta}{Metadata for pseudobulk samples}
#'   \item{dge}{DGEList object}
#'   \item{design}{Design matrix}
#'   \item{contrast_levels}{Factor levels for contrasts}
#'
#' @export
pb_prepare_edgeR <- function(seurat_obj,
  ...
  pb_utils_validate_metadata(seurat_obj, c(sample_col, cluster_col, group_col))
  ...
  if (nrow(pb) == 0 || ncol(pb) == 0) {
    stop("AggregateExpression resulted in empty pseudo-bulk matrix. Check group.by columns.")
  }
  ...
  meta_pb <- tryCatch({
    ...
    stop("Cannot parse pseudo-bulk column names ('", col_example, "') into 'patient", sep, "ctype' format. ",
         "Check AggregateExpression group.by order. Error: ", e$message)
  })
  ...
  # Apply g-prefixing logic for numeric sample IDs
  ...
  if (any(needs_g_prefix_flags) && verbose) {
    message("Debug (pb_prepare_edgeR): Applying 'g' prefix to numeric-like values in '", sample_col, "'.")
  }
  original_meta_subset[[sample_col]][needs_g_prefix_flags] <- paste0("g", original_meta_subset[[sample_col]][needs_g_prefix_flags])

  # Check for samples mapped to multiple distinct groups
  ...
  if(nrow(sample_to_multiple_groups_check) > 0 && verbose) {
    warning("pb_prepare_edgeR: Multiple distinct groups found for the same sample ID. Affected sample(s): ",
            paste(utils::head(sample_to_multiple_groups_check[[sample_col]], 5), collapse=", "),
            ". Using first encountered group for each sample.")
  }

  # Create sample_group_map
  ...
  if (verbose) {
    message("Debug (pb_prepare_edgeR): sample_group_map created with ", nrow(sample_group_map), " unique entries.")
  }
  ...
  meta_pb <- meta_pb_joined

  # Check for NAs in group mapping
  if (sum(is.na(meta_pb[[group_col]]))) {
    message("Debug: WARNING - Group mapping resulted in NAs for some pseudo-bulk samples.")
    failed_patients <- meta_pb %>% dplyr::filter(is.na(!!rlang::sym(group_col))) %>% dplyr::pull(patient) %>% unique()
    message("Debug: Patient IDs that failed to map: ", paste(utils::head(failed_patients, 10), collapse=", "))
  } else {
    if (verbose) message("Debug (pb_prepare_edgeR): Group mapping successful for all pseudo-bulk samples.")
  }
  ...
  # Match row order of meta_pb to column order of pb
  ...
  if ("pb_sample_id" %in% colnames(meta_pb)) {
    ...
    warning("pb_prepare_edgeR: 'pb_sample_id' column not found. Cannot set rownames for DGEList samples.")
  }
  ...
  if (sum(keep) == 0) {
    warning("filterByExpr resulted in zero genes. Lower min.count or check data.")
  }
...
#' Create Advanced Pseudobulk Expression Matrix
#'
#' Creates pseudobulk expression matrix with flexible aggregation options,
#' including support for additional stratification and covariates.
#'
#' @param seurat_obj Seurat object
#' @param sample.by Column name defining biological samples
...
#' @param min.cells Minimum cells per pseudobulk sample (default: 10)
#'
#' @return List with:
#'   \item{matrix}{Pseudobulk expression matrix}
#'   \item{metadata}{Metadata for pseudobulk samples}
#'
#' @export
pb_prepare_matrix <- function(seurat_obj,
                               sample.by,
                               group.by,
                               aggregate.by = NULL,
                               covariates = NULL,
...
# ==============================================================================
# STATISTICAL BACKEND FUNCTIONS
# ==============================================================================

#' Run Advanced edgeR Analysis on Pseudobulk Data
#'
#' Performs differential expression analysis using edgeR on pseudobulk data
#' with support for covariates and custom design formulas.
#'
#' @param pb_matrix Pseudobulk expression matrix
#' @param metadata Metadata for pseudobulk samples
#' @param group.by Column in metadata defining groups
#' @param ident.1 Identity of first group (target)
#' @param ident.2 Identity of second group (reference, default: NULL for all others)
#' @param covariates Additional metadata columns to include as covariates
...
#'
#' @export
pb_run_edgeR <- function(pb_matrix,
                         metadata,
                         group.by,
                         ident.1,
                         ident.2 = NULL,
  ...
  # Ensure condition columns in design matrix
  ...
  de_results <- pb_utils_format_results(de_results, method = "edgeR", add_gene_col = TRUE)
...
#'
#' Performs differential expression analysis using DESeq2 on pseudobulk data
#' with support for covariates and custom design formulas.
#'
#' @param pb_matrix Pseudobulk expression matrix
#' @param metadata Metadata for pseudobulk samples
...
#'
#' @export
pb_run_DESeq2 <- function(pb_matrix,
                          metadata,
                          group.by,
                          ident.1,
                          ident.2 = NULL,
  ...
  de_results <- pb_utils_format_results(de_results, method = "DESeq2", add_gene_col = TRUE)
...
#' Pseudobulk Differential Expression Analysis with edgeR
#'
#' Performs pseudobulk differential gene expression analysis at various levels
#' (overall, per-cluster, or specific cluster) using edgeR.
#'
#' @param analysis_level Analysis level: "overall", "per_cluster", or "specific_cluster"
#' @param contrast Contrast vector for glmQLFTest
#' @param target_cluster Target cluster name (required for "specific_cluster" mode)
#' @param min_samples_per_group_cluster Minimum samples per group for cluster analysis
#' @param min_count Minimum count for filterByExpr
#' @param ... Arguments for pb_prepare_edgeR or prepared_data
#' @param verbose Print progress messages
#'
#' @return Data frame with DEG results
#'
#' @export
pb_deg_edgeR <- function(analysis_level = c("overall", "per_cluster", "specific_cluster"),
                               contrast,
  ...
  # --- 1. 데이터 준비 (pb_prepare_edgeR 호출 또는 기존 데이터 사용) ---
  ...
  if ("prepared_data" %in% names(dot_args) &&
    ...
    pb   <- prep_data$pb
    meta <- prep_data$meta # This meta is meta_pb from pb_prepare_edgeR
    dge  <- prep_data$dge
    ...
    if (verbose) message("Preparing pseudo-bulk data using pb_prepare_edgeR...")
    ...
    group_col <- dot_args$group_col # Crucial: get group_col for pb_prepare_edgeR
    
    all_args_for_prep <- c(dot_args, list(verbose = verbose))
    # Pass min_count from pb_deg_edgeR to pb_prepare_edgeR if not already in dot_args
    if (!"min_count" %in% names(all_args_for_prep)) {
      all_args_for_prep$min_count <- min_count # Use min_count from pb_deg_edgeR's signature
    }
    
    prep_data_list_internal <- do.call(pb_prepare_edgeR, all_args_for_prep)
  ...
  if (analysis_level == "per_cluster" || analysis_level == "specific_cluster") {
    ...
    for (cl_name in loop_over_these_clusters) { # 루프 변수 이름 변경 (예: cl_name)
      ...
      # 이 부분에서 min_count는 pb_deg_edgeR 함수의 인수로 받은 min_count를 사용합니다.
...
pb_deg_cluster <- function(sobj,
  ...
  if (verbose) message("Starting pb_deg_cluster analysis...")
  ...
  for (current_target_cluster_name in clusters_to_iterate) {
    ...
    if (comparison_mode == "FindAllMarkers" || comparison_mode == "OneVsRest") {
      ...
      # Create a temporary sobj for this specific comparison to pass to pb_prepare_edgeR
    ...
    # --- 3. Perform DEG ---
    if (test.use == "wilcox") { # Assuming "wilcox" implies using the edgeR backend from pb_deg_edgeR
      if (verbose) message("  Using edgeR via pb_deg_edgeR...")
      
      # Check for sufficient samples in each group within temp_group_col *after pseudo-bulking*
      # This is implicitly handled by pb_prepare_edgeR's internal checks or edgeR itself.
      # We can add an explicit check if needed on the output of pb_prepare_edgeR$meta.
      
      # Prepare data for edgeR
      # `cluster_col` in `pb_prepare_edgeR` should be the original `group.by`
      ...
      prepared_data_for_deg <- tryCatch(
        do.call(pb_prepare_edgeR, prep_args),
        error = function(e) {
          message("    Error in pb_prepare_edgeR for ", current_target_cluster_name, ": ", e$message)
          return(NULL)
        }
      )
      ...
      patient_col_in_meta_pb <- "patient"     # As defined in pb_prepare_edgeR's separate step
      ...
      if (!grouping_col_in_meta_pb %in% colnames(prepared_data_for_deg$meta)) {
        message("    CRITICAL WARNING for '", current_target_cluster_name,
                ...
                ". This indicates a problem in `pb_prepare_edgeR` or how `temp_group_col` was defined/passed.",
      ...
      deg_results_raw <- tryCatch({
        ...
        message("    Error in pb_deg_edgeR for ", current_target_cluster_name, ": ", e$message)
        return(NULL)
      })
      
      if (is.null(deg_results_raw) || nrow(deg_results_raw) == 0) {
        if (verbose) message("    No DEG results from pb_deg_edgeR for ", current_target_cluster_name)
...
# --- Func END ----
pb_deg_FindMarkers <- function(seurat_obj,
                                   ident.1,
                                   ident.2 = NULL,
                                   group.by = "seurat_clusters",
                                   sample.by,
...
# --- Func END ----
pb_deg_FindAllMarkers <- function(seurat_obj,
                                      group.by = "seurat_clusters",
                                      sample.by,
                                      covariates = NULL,
                                      aggregate.by = NULL,
...
# --- Func END ----
pb_linear_compare_slopes <- function(results_df,
                                      gene_col = "gene",
                                      group_col = "group",
                                      slope_col = "slope",
                                      se_col = "slope_se",
