...
validate_cci_inputs <- function(sobj, cluster_col, deg_df, receiver_cluster, sender_clusters = NULL) {
  ...
  if (is.null(deg_df)) {
    stop("`deg_df` cannot be NULL. Please provide a DEG dataframe or ensure automatic DEG computation is enabled.")
  }
...
#' If `top_n` is specified, cutoff filters are ignored and top N DEGs are selected based on logFC.
#'
#' @param deg_df Data frame with DEG results
#' @param receiver_cluster Character string, the receiver cluster ID
#' @param p_val_adj_cutoff Numeric, adjusted p-value cutoff (default: 0.05). Ignored if `top_n` is specified.
#' @param logfc_cutoff Numeric, log fold-change cutoff (default: 0.25). Ignored if `top_n` is specified.
#' @param only_upregulated Logical, whether to only include upregulated genes (default: TRUE)
#' @param top_n Integer or NULL. If specified, cutoff filters are ignored and top N DEGs are selected by logFC (descending). If NULL, cutoff filters are applied.
#'
...
extract_receiver_degs <- function(deg_df, receiver_cluster, 
  ...
  # If top_n is specified, ignore cutoffs and select top N by logFC
  if (!is.null(top_n) && top_n > 0) {
    # Filter for upregulated if requested
    if (only_upregulated) {
      receiver_degs <- receiver_degs %>%
        dplyr::filter(!!dplyr::sym(logfc_col) > 0)
    }
    
    # Sort by p-value (ascending) first, then by logFC (descending)
    if (!is.null(pval_col)) {
      receiver_degs <- receiver_degs %>%
        dplyr::arrange(!!dplyr::sym(pval_col), dplyr::desc(!!dplyr::sym(logfc_col)))
    } else {
      receiver_degs <- receiver_degs %>%
        dplyr::arrange(dplyr::desc(!!dplyr::sym(logfc_col)))
    }
    
    # Select top N
    receiver_degs <- receiver_degs %>%
      dplyr::slice_head(n = min(top_n, nrow(receiver_degs)))
    
    return(receiver_degs)
  }
  
  # Otherwise, apply cutoff filters (original behavior)
  # Filter by logFC
  if (only_upregulated) {
    receiver_degs <- receiver_degs %>%
  ...
  # Apply p-value filter if column exists and cutoff is meaningful (< 1.0)
  # If cutoff >= 1.0, skip p-value filtering (effectively no filter)
  if (!is.null(pval_col) && p_val_adj_cutoff < 1.0) {
    receiver_degs <- receiver_degs %>%
