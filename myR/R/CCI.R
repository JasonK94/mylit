  ...
  analysis_results <- withCallingHandlers(
    {
            ...
            tryCatch(
              {
                ...
                writeLines(
                  c(
                    "# NicheNet Analysis Arguments (fallback)",
      ...
      sig_network <- nn_data$sig_network
      gr_network <- nn_data$gr_network
      weighted_networks <- nn_data$weighted_networks
      ligand_target_matrix <- nn_data$ligand_target_matrix
      ligand_tf_matrix <- nn_data$ligand_tf_matrix

      # Calculate weighted_networks_lr if not present
      if (!is.null(nn_data$weighted_networks_lr)) {
        weighted_networks_lr <- nn_data$weighted_networks_lr
      } else {
        weighted_networks_lr <- weighted_networks$lr_sig %>%
          dplyr::inner_join(lr_network %>% dplyr::distinct(from, to), by = c("from", "to"))
      }
      ...
      ligands_db <- lr_network %>%
        dplyr::pull(from) %>%
        unique()
      receptors_db <- lr_network %>%
        dplyr::pull(to) %>%
        unique()
      ...
      best_upstream_ligands <- ligand_activities %>%
        dplyr::top_n(min(top_n_ligands, nrow(ligand_activities)), aupr_corrected) %>%
      ...
      if (nrow(active_ligand_target_links_df) > 0) {
        ...
        if (is.null(active_ligand_target_links_matrix) || nrow(active_ligand_target_links_matrix) == 0 || ncol(active_ligand_target_links_matrix) == 0) {
          warning(
            "NN_Warning: Ligand-target matrix is empty/small after ", max_attempts, " attempts with cutoffs down to ", current_cutoff, ". ",
            "Input had ", nrow(active_ligand_target_links_df), " links. ",
            "Consider checking if DEGs are relevant for NicheNet analysis."
          )
          p_ligand_target_network <- ggplot2::ggplot() +
            ggplot2::labs(title = paste0("No L-T links for heatmap after preparation (cutoff=", current_cutoff, ")"))
        } else {
          ligands_for_ordering <- base::intersect(best_upstream_ligands, colnames(active_ligand_target_links_matrix)) %>% rev()
          targets_for_ordering <- active_ligand_target_links_df$target %>%
            unique() %>%
            base::intersect(rownames(active_ligand_target_links_matrix))
          ...
          if (length(order_targets_final_intersect) == 0 || length(order_ligands_final_intersect) == 0) {
            ...
            p_ligand_target_network <- ggplot2::ggplot() +
              ggplot2::labs(title = "No common targets/ligands for L-T heatmap.")
          } else {
        ...
        p_ligand_target_network <- ggplot2::ggplot() +
          ggplot2::labs(title = "No active L-T links found.")
      }
        ...
        if (length(valid_ligands_in_df) > 0 && length(valid_receptors_in_df) > 0) {
          ...
          if (nrow(lr_network_top_df_filtered) > 0) {
            ...
            if (nrow(lr_network_top_matrix) > 1 && ncol(lr_network_top_matrix) > 1) {
              ...
              dist_ligands <- stats::dist(t(lr_network_top_matrix), method = "binary")
            ...
            rownames(vis_ligand_receptor_network) <- make.names(rownames(vis_ligand_receptor_network))
            colnames(vis_ligand_receptor_network) <- make.names(colnames(vis_ligand_receptor_network))
            ...
            warning("NN_Warning: Not enough valid L-R pairs for heatmap after filtering.")
            p_ligand_receptor_network <- ggplot2::ggplot() +
              ggplot2::labs(title = "Not enough valid L-R pairs for heatmap.")
          }
        } else {
          warning("NN_Warning: Not enough valid L-R pairs for heatmap after initial filtering.")
          p_ligand_receptor_network <- ggplot2::ggplot() +
            ggplot2::labs(title = "Not enough valid L-R pairs for heatmap.")
        }
      } else {
        warning("NN_Warning: No L-R interactions to display (no best ligands/receptors).")
        p_ligand_receptor_network <- ggplot2::ggplot() +
          ggplot2::labs(title = "No L-R interactions to display.")
      }

      # --- VIII. Additional Plots ---
      p_hist_lig_activity <- ggplot2::ggplot(ligand_activities, ggplot2::aes(x = aupr_corrected)) +
        ggplot2::geom_histogram(color = "black", fill = "darkorange", bins = 30) +
        ggplot2::geom_vline(ggplot2::aes(xintercept = min(ligand_activities %>% dplyr::top_n(min(top_n_ligands, nrow(ligand_activities)), aupr_corrected) %>% dplyr::pull(aupr_corrected))), color = "red", linetype = "dashed", linewidth = 1) +
        ggplot2::labs(x = "Ligand Activity (AUPR corrected)", y = "# Ligands") +
        ...
        if (requireNamespace("circlize", quietly = TRUE) &&
          ...
          if (exists("lr_network_top_df_large") && is.data.frame(lr_network_top_df_large) && nrow(lr_network_top_df_large) > 0 && all(c("from", "to", "weight") %in% colnames(lr_network_top_df_large))) {
            ...
            if (nrow(circos_lr_data_raw) > 0) {
              ...
              ordered_ligands <- ligand_df_circos %>%
                dplyr::arrange(color_group, item) %>%
                dplyr::pull(item)
              ordered_receptors <- receptor_df_circos %>%
                dplyr::arrange(item) %>%
                dplyr::pull(item)
              ...
              tryCatch(
                {
                  if (!is.null(current_run_output_dir)) {
                    ...
                    grDevices::pdf(file.path(current_run_output_dir, "NicheNet_Circos_LR_no_legend.pdf"), width = circos_pdf_width, height = circos_pdf_height)
                    do_circos_plotting(show_legend = FALSE)
                    grDevices::dev.off()
                    grDevices::png(file.path(current_run_output_dir, "NicheNet_Circos_LR_no_legend.png"), width = circos_png_width, height = circos_png_height, res = circos_png_res)
                    do_circos_plotting(show_legend = FALSE)
                    grDevices::dev.off()

                    # Save plot with legend
                    if (circos_show_legend && length(legend_labels) > 0) {
                      grDevices::pdf(file.path(current_run_output_dir, "NicheNet_Circos_LR_with_legend.pdf"), width = circos_pdf_width, height = circos_pdf_height)
                      do_circos_plotting(show_legend = TRUE)
                      grDevices::dev.off()
                      grDevices::png(file.path(current_run_output_dir, "NicheNet_Circos_LR_with_legend.png"), width = circos_png_width, height = circos_png_height, res = circos_png_res)
                      do_circos_plotting(show_legend = TRUE)
                      grDevices::dev.off()

                      # Save legend only
                      grDevices::pdf(file.path(current_run_output_dir, "NicheNet_Circos_LR_legend_only.pdf"), width = 4, height = 6)
                      do_legend_only()
                      grDevices::dev.off()
                      grDevices::png(file.path(current_run_output_dir, "NicheNet_Circos_LR_legend_only.png"), width = 400, height = 600, res = circos_png_res)
                      do_legend_only()
                      grDevices::dev.off()
                    }
      ...
      objects_to_clear <- c(
        "ligand_activities", "active_ligand_target_links_df", "active_ligand_target_links_matrix",
...
prepare_nichenet_circos_data <- function(lr_network_top_df_large,
  ...
  if (is.null(ligand_sender_map)) {
    ...
    ligand_sender_map <- sapply(unique_ligands_in_circos, function(lig) {
      ...
      if (length(expressing_senders) > 0) {
        return(expressing_senders[1])
      }
