...
run_multinichenet_analysis <- function(sobj,
    ...
    # Input: "Stroke-HC" or "'Stroke-HC'" or c("Stroke-HC", "HC-Stroke")
    ...
    # CRITICAL: multinichenetr expects contrasts_oi as apostrophe-wrapped string
    # e.g., "'Stroke-HC'" for single contrast, "'A-B','C-D'" for multiple.
    # Apostrophes make limma::makeContrasts() treat it as a quoted string literal,
    # preserving "Stroke-HC" as the column name. Without apostrophes, makeContrasts
    # deparses the bare expression to "Stroke - HC" (with spaces), causing a
    # mismatch with contrast_tbl in inner_join → group_prioritization_tbl = 0 rows.
    contrasts_oi <- paste0("'", raw_contrasts, "'", collapse = ",")

    if (verbose) {
        message("Contrast table:")
        print(contrast_tbl)
        message("contrasts_oi (apostrophe-wrapped): ", contrasts_oi)
    }
