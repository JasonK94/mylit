# Production Analysis - Dream Only
...
if (!exists("run_deg_consensus")) {
    ...
    source("myR/R/deg_consensus/deg_methods_dream.R")
...
# Impute BMI for NAs (Median Imputation) - although not used for dream for now
...
group_col <- "g3"
sample_col <- "hos_no"
# Dream Test: batch_id='set', covar='sex, age'
# Dream will handle set as random effect (1|set)
covariates <- c("sex", "age")
batch_var <- "set"
cores <- 6

# Helper
run_dream_safe <- function(obj, label, cluster_col, out_subdir) {
    out_dir <- file.path(base_out_dir, out_subdir)
    out_file <- file.path(out_dir, paste0("results_", label, "_dream.qs"))
    ...
    message(sprintf("\n>>> Starting Dream: %s (Cluster: %s)", label, cluster_col))
...
# Levels
run_dream_safe(sobj_full, "Global", "global_group", "Level1_Global")
run_dream_safe(sobj_full, "Broad", "anno3big", "Level2_Broad")
...
for (bt in broad_types) {
    ...
    run_dream_safe(sobj_sub, label, "anno3", paste0("Level3_Fine_", label))
    rm(sobj_sub)
    gc()
}
