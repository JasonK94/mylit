#!/usr/bin/env Rscript
# Step 2: LogNormalize normalization and clustering
# Usage: Rscript pipe2_nmz_clustering.R --config <config_path> --run_id <run_id> --input_step <step> --output_step <step>
...
log_message(sprintf("Step %d: Starting LogNormalize normalization and clustering", opt$output_step), log_list)
...
# Save results
if (length(sl) > 0) {
  output_filename <- get_param("output_step2_nmz", config_list, "step2_nmz_list.qs")
