...
# 1. HTO/SNP Mapping Logic
# Replaced by robust metadata joining from config and clinical data
meta <- merged@meta.data

# Ensure join key exists
if ("sample_id" %in% colnames(meta)) {
  meta$join_key <- meta$sample_id
} else if ("Best_Sample" %in% colnames(meta)) {
...
# Try to get dir_meta_data from config (manifest) if not in params
clinical_meta_path <- get_param("dir_meta_data", config_list, NULL)
if (is.null(clinical_meta_path) && !is.null(config_list$config) && "dir_meta_data" %in% colnames(config_list$config)) {
  # Use the first non-NA value from the manifest
  paths <- config_list$config$dir_meta_data
  paths <- paths[!is.na(paths) & paths != ""]
  if (length(paths) > 0) {
    clinical_meta_path <- paths[1]
  }
}
if (is.null(clinical_meta_path)) {
  clinical_meta_path <- "/home/user3/Clinical information/meta_data_prep_251113.csv"
}
...
if (file.exists(clinical_meta_path)) {
  ...
  # Check for duplicates in clinical metadata
  if (any(duplicated(meta_clinical$sample_no))) {
    log_message("WARNING: Duplicate sample_no found in clinical metadata. Keeping first occurrence.", log_list, level = "WARNING")
    meta_clinical <- meta_clinical[!duplicated(meta_clinical$sample_no), ]
  }
  ...
  # Use distinct to avoid many-to-many if Seurat has multiple cells per sample (which is expected)
  # But meta_clinical should be unique per sample_no
  ...
  log_message(sprintf("WARNING: Clinical metadata file not found: %s", clinical_meta_path), log_list, level = "WARNING")
}
...
if (!is.null(config_list$config)) {
  ...
  std_cols <- c(
    "no", "name", "patient_name", "contamination_risk", "sample_name", "gem_name",
  ...
  integrated <- tryCatch(
    {
      ...
      # Downstream
      merged <- FindNeighbors(merged, reduction = "integrated.scvi", dims = dims, verbose = FALSE)
      merged <- FindClusters(merged, resolution = resolution, verbose = FALSE)
      merged <- RunUMAP(merged, reduction = "integrated.scvi", dims = dims, verbose = FALSE)
      ...
      merged # Return merged as integrated
    },
    error = function(e) {
