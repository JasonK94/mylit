...
auto_estimate_with_retry <- function(sc, sample_name, plot_path) {
  ...
  repeat {
    log_message(sprintf(
      "  autoEstCont attempt %d for %s (tfidfMin=%.2f, soupQuantile=%.2f)",
      attempt, sample_name, current_tfidf, current_soup_q
    ), log_list)
    ...
    tryCatch(
      ...
      error = function(e) {
        ...
        log_message(sprintf(
          "  ERROR: autoEstCont attempt %d failed for %s: %s",
          attempt, sample_name, e$message
    ...
    if (success) {
      ...
      if (attempt > 1) {
        log_message(sprintf(
          "  autoEstCont succeeded after lowering thresholds to tfidfMin=%.2f, soupQuantile=%.2f",
          current_tfidf, current_soup_q
        ), log_list)
      }
    ...
    if (current_tfidf <= soupx_tfidf_min_floor && current_soup_q <= soupx_soup_quantile_floor) {
      log_message(sprintf("  autoEstCont failed for %s at minimum thresholds. Skipping SoupX correction.", sample_name), log_list, level = "WARNING")
      return(NULL)
    }
    ...
    if (new_tfidf == current_tfidf && new_soup_q == current_soup_q) {
      log_message(sprintf("  autoEstCont failed for %s and parameters cannot be reduced further. Skipping SoupX correction.", sample_name), log_list, level = "WARNING")
      return(NULL)
    }
  ...
  tryCatch(
    {
      ...
      if (is.null(est_result)) {
        log_message(sprintf("  Skipping SoupX for %s due to autoEstCont failure (keeping original counts)", sample_name), log_list, level = "WARNING")
        next
      }
      sc <- est_result$sc
      ...
      tryCatch(
        ...
        error = function(e) {
          ...
          log_message(sprintf(
            "  WARNING: Failed to generate Marker Distribution plot for %s: %s. %s",
            sample_name, e$message, suggestion
    ...
    error = function(e) {
      log_message(sprintf(
        "!!! ERROR in step %d processing %s: %s !!!",
