...
# 개선된 FindMarkers-like 함수
findMarkersEnhanced <- function(data_norm, SP, group_col, ident.1, ident.2 = NULL,
                               test.use = "wilcox", min.pct = 0.1, 
                               logfc.threshold = 0.25, only.pos = FALSE) {
  
  # 그룹 정의
  if(is.null(ident.2)) {
    group1 <- SP[[group_col]] == ident.1
    group2 <- SP[[group_col]] != ident.1
  } else {
    group1 <- SP[[group_col]] == ident.1
    group2 <- SP[[group_col]] == ident.2
  }
  
  # 각 유전자에 대해 계산
  results <- pbapply::pblapply(rownames(data_norm), function(gene) {
    expr1 <- as.numeric(data_norm[gene, group1])
    expr2 <- as.numeric(data_norm[gene, group2])
    ...
    # min.pct 필터
    if(max(pct1, pct2) < min.pct) return(NULL)
    
    # log2FC 계산
    mean1 <- mean(expr1)
    mean2 <- mean(expr2)
    log2fc <- log2((mean1 + 1e-9) / (mean2 + 1e-9))
    
    # logfc.threshold 필터
    if(abs(log2fc) < logfc.threshold) return(NULL)
    if(only.pos && log2fc < 0) return(NULL)
    
    # 통계 검정
    if(test.use == "wilcox") {
      test_res <- wilcox.test(expr1, expr2)
    } else if(test.use == "t") {
      test_res <- t.test(expr1, expr2)
    }
...
pre&post가 있는 것만..
```{r}
valid_emrids <- data_seurat@meta.data %>%
  mutate(unique_id= paste0(emrid, ck)) %>%
  group_by(unique_id) %>%
...
valid_emrids2 <- data_seurat@meta.data %>%
  mutate(unique_id= paste0(emrid, ck)) %>%
  group_by(emrid) %>%
  filter(all(c("pre", "post") %in% treatment)) %>%
  filter(all(c("TRUE","FALSE")%in% ck)) %>%
  dplyr::select(c("unique_id", "emrid", "ck", "treatment")) #%>% # 조건을 만족하는 emrid만 벡터 형태로 추출
  # unique()       # 중복 제거

data_seurat_original=data_seurat
data_seurat=subset(data_seurat, SegmentDisplayName %in% valid_emrids$SegmentDisplayName)
...
# NR vs R
table(data_seurat$drug, data_seurat$treatment, data_seurat$`mucosal healing`)
table(data_seurat$drug, data_seurat$treatment, data_seurat$`histologic Endoscopic improvement`)
table(data_seurat$drug, data_seurat$treatment, data_seurat$`Endo remission`)
table(data_seurat$drug, data_seurat$treatment, data_seurat$`Deep mucosal healing`)

# NR vs R - detail

table(data_seurat$drug, data_seurat$treatment, data_seurat$ck, data_seurat$`mucosal healing`)
table(data_seurat$drug, data_seurat$treatment, data_seurat$ck, data_seurat$`histologic Endoscopic improvement`)
table(data_seurat$drug, data_seurat$treatment, data_seurat$ck, data_seurat$`Endo remission`)
table(data_seurat$drug, data_seurat$treatment, data_seurat$ck, data_seurat$`Deep mucosal healing`)

# pre vs post

table(data_seurat$drug, data_seurat$treatment, data_seurat$ck)
table(data_seurat$drug, data_seurat$treatment, data_seurat$ck)
table(data_seurat$drug, data_seurat$treatment, data_seurat$ck)
table(data_seurat$drug, data_seurat$treatment, data_seurat$ck)
```
...
#heal, drug_r, drug_t, ck_heal, ck_r, ck_t

# data_seurat2=AddMultipleModuleScores(data_seurat, genes_gut)
# plots=PlotModuleScoreHeatmap()
plots1=PlotModuleScoreHeatmap(data_ck, gene_sets = genes_gut, group="treatment", assay="RNA")
plots2=PlotModuleScoreHeatmap(data_str, gene_sets = genes_gut, group="treatment", assay="RNA")

plots3=PlotModuleScoreHeatmap(data_ck, gene_sets = genes_gut, group="drug_t", assay="RNA")
plots4=PlotModuleScoreHeatmap(data_str, gene_sets = genes_gut, group="drug_t", assay="RNA")
plots31=PlotModuleScoreHeatmap(data_ck, gene_sets = genes_gut_ibd_init, group="drug_t", assay="RNA")
plots41=PlotModuleScoreHeatmap(data_str, gene_sets = genes_gut_ibd_init, group="drug_t", assay="RNA")
plots32=PlotModuleScoreHeatmap(data_seurat, gene_sets = genes_gut_ibd_init, group="drug_t", assay="RNA")

plots33=PlotModuleScoreHeatmap(data_ck, gene_sets = genes_gut_ibd_init, group="drug_r", assay="RNA")
plots43=PlotModuleScoreHeatmap(data_str, gene_sets = genes_gut_ibd_init, group="drug_r", assay="RNA")

plots5=PlotModuleScoreHeatmap(data_ck, gene_sets = genes_gut, group="r_t", assay="RNA")
plots6=PlotModuleScoreHeatmap(data_str, gene_sets = genes_gut, group="r_t", assay="RNA")
plots51=PlotModuleScoreHeatmap(data_ck, gene_sets = neutrophil_markers, group="r_t", assay="RNA")
plots61=PlotModuleScoreHeatmap(data_str, gene_sets = neutrophil_markers, group="r_t", assay="RNA")

plots7=PlotModuleScoreHeatmap(data_ck, gene_sets = genes_gut, group="heal_t", assay="RNA")
plots8=PlotModuleScoreHeatmap(data_str, gene_sets = genes_gut, group="heal_t", assay="RNA")

plots9=PlotModuleScoreHeatmap(data_ck, gene_sets = genes_gut, group="heal_t", assay="RNA")
plots10=PlotModuleScoreHeatmap(data_str, gene_sets = genes_gut, group="heal_t", assay="RNA")

plots11=PlotModuleScoreHeatmap(data_ck, gene_sets = genes_gut, group="drug_r_t", assay="RNA")
plots12=PlotModuleScoreHeatmap(data_str, gene_sets = genes_gut, group="drug_r_t", assay="RNA")

# response - heal =0, 1
plots13=PlotModuleScoreHeatmap(data_ck_pre, gene_sets = genes_gut, group="response", assay="RNA")
plots14=PlotModuleScoreHeatmap(data_str_pre, gene_sets = genes_gut_ibd_init, group="response", assay="RNA")
plots15=PlotModuleScoreHeatmap(data_ck_post, gene_sets = genes_gut, group="response", assay="RNA")
plots16=PlotModuleScoreHeatmap(data_str_post, gene_sets = genes_gut_ibd_init, group="response", assay="RNA")
plots17=PlotModuleScoreHeatmap(data_ck, gene_sets = genes_gut, group="response", assay="RNA")
plots18=PlotModuleScoreHeatmap(data_str, gene_sets = genes_gut_ibd_init, group="response", assay="RNA")



# ER
plots19=PlotModuleScoreHeatmap(data_ck_pre, gene_sets = genes_gut, group="Endo remission", assay="RNA")
plots20=PlotModuleScoreHeatmap(data_str_pre, gene_sets = genes_gut_ibd_init, group="Endo remission", assay="RNA")
plots21=PlotModuleScoreHeatmap(data_ck_post, gene_sets = genes_gut, group="Endo remission", assay="RNA")
plots22=PlotModuleScoreHeatmap(data_str_post, gene_sets = genes_gut_ibd_init, group="Endo remission", assay="RNA")
plots23=PlotModuleScoreHeatmap(data_ck, gene_sets = genes_gut, group="Endo remission", assay="RNA")
plots24=PlotModuleScoreHeatmap(data_str, gene_sets = genes_gut_ibd_init, group="Endo remission", assay="RNA")
...
data_str=subset(data_seurat, ck==FALSE)
...
data_inf=subset(data_seurat, drug=="Infliximab")
data_ust=subset(data_seurat, drug=="Ustekinumab")
data_ved=subset(data_seurat, drug=="Vedolizumab")

data_ck_inf=subset(data_ck, drug=="Infliximab")
data_ck_ust=subset(data_ck, drug=="Ustekinumab")
data_ck_ved=subset(data_ck, drug=="Vedolizumab")
data_str_inf=subset(data_str, drug=="Infliximab")
data_str_ust=subset(data_str, drug=="Ustekinumab")
data_str_ved=subset(data_str, drug=="Vedolizumab")

data_ck_pre_inf=subset(data_ck_pre, drug=="Infliximab")
data_ck_pre_ust=subset(data_ck_pre, drug=="Ustekinumab")
data_ck_pre_ved=subset(data_ck_pre, drug=="Vedolizumab")
data_ck_post_inf=subset(data_ck_post, drug=="Infliximab")
data_ck_post_ust=subset(data_ck_post, drug=="Ustekinumab")
data_ck_post_ved=subset(data_ck_post, drug=="Vedolizumab")
data_str_pre_inf=subset(data_str_pre, drug=="Infliximab")
data_str_pre_ust=subset(data_str_pre, drug=="Ustekinumab")
data_str_pre_ved=subset(data_str_pre, drug=="Vedolizumab")
data_str_post_inf=subset(data_str_post, drug=="Infliximab")
data_str_post_ust=subset(data_str_post, drug=="Ustekinumab")
data_str_post_ved=subset(data_str_post, drug=="Vedolizumab")
...
data.nr <- subset(data_seurat, subset = Endo_remission == 0)
data.r <- subset(data_seurat, subset = Endo_remission == 1)

data.nr.ck=subset(data.nr, ck==TRUE)
data.nr.str=subset(data.nr, ck==FALSE)
data.nr.ck_inf=subset(data.nr.ck, drug=="Infliximab")
data.nr.ck_ust=subset(data.nr.ck, drug=="Ustekinumab")
data.nr.ck_ved=subset(data.nr.ck, drug=="Vedolizumab")
data.nr.str_inf=subset(data.nr.str, drug=="Infliximab")
data.nr.str_ust=subset(data.nr.str, drug=="Ustekinumab")
data.nr.str_ved=subset(data.nr.str, drug=="Vedolizumab")

data.r.ck=subset(data.r, ck==TRUE)
data.r.str=subset(data.r, ck==FALSE)
data.r.ck_inf=subset(data.r.ck, drug=="Infliximab")
data.r.ck_ust=subset(data.r.ck, drug=="Ustekinumab")
data.r.ck_ved=subset(data.r.ck, drug=="Vedolizumab")
data.r.str_inf=subset(data.r.str, drug=="Infliximab")
data.r.str_ust=subset(data.r.str, drug=="Ustekinumab")
data.r.str_ved=subset(data.r.str, drug=="Vedolizumab")
...
#batch_key_to_use="heal"

reduction_to_use="UMAP_harmony"
#batch_key_to_use="SlideName"
batch_key_to_use="drug"
reduction_keys=names(sobj@reductions[[reduction_to_use]])


umap_df <- Embeddings(sobj, reduction_to_use) %>% 
  as.data.frame() %>% 
  # 열 이름 확인: 보통 "UMAP_1", "UMAP_2" 라고 나옵니다
  #rename(umap_1 = umap_1, umap_2 = umap_2) %>% 
  # 메타데이터에서 heal 값 붙이기
  mutate(batch = sobj@meta.data[[batch_key_to_use]]) %>%
  mutate(umap_1 = !!sym(reduction_keys[[1]]),
    umap_2 = !!sym(reduction_keys[[2]])
    )
...
centroids <- umap_df %>% 
  group_by(batch) %>% 
  summarise(
    umap_1 = mean(!!sym(reduction_keys[[1]])),
    umap_2 = mean(!!sym(reduction_keys[[2]]))
  )
...
p <- ggplot(umap_df, aes(x = umap_1, y = umap_2, color = factor(batch))) +
  geom_point(alpha = 0.6, size = 0.5) +
  scale_color_brewer(palette = "Set1", name = batch_key_to_use) +
  theme_classic() +
  labs(title = "UMAP with heal group centroids")
...
p +
  ...
  geom_text_repel(
    data = centroids,
    aes(x = umap_1, y = umap_2, label = batch),
    size = 5,
    fontface = "bold",
    color = "black"
  )
...
m.pp=FindMarkers(data_seurat,group.by="treatment",ident.1="post")
m.pp.ck=FindMarkers(data_ck,group.by="treatment",ident.1="post")
m.pp.str=FindMarkers(data_str,group.by="treatment",ident.1="post")
m.pp.ck.inf=FindMarkers(data_ck_inf,group.by="treatment",ident.1="post")
m.pp.ck.ust=FindMarkers(data_ck_ust,group.by="treatment",ident.1="post")
m.pp.ck.ved=FindMarkers(data_ck_ved,group.by="treatment",ident.1="post")
m.pp.str.inf=FindMarkers(data_str_inf,group.by="treatment",ident.1="post")
m.pp.str.ust=FindMarkers(data_str_ust,group.by="treatment",ident.1="post")
m.pp.str.ved=FindMarkers(data_str_ved,group.by="treatment",ident.1="post")

# m.pp2=FindMarkers(data_seurat,group.by="treatment",ident.1="post", test.use="roc")
# m.pp.ck2=FindMarkers(data_ck,group.by="treatment",ident.1="post", test.use="roc")
# m.pp.str2=FindMarkers(data_str,group.by="treatment",ident.1="post", test.use="roc")
# m.pp.ck.inf2=FindMarkers(data_ck_inf,group.by="treatment",ident.1="post", test.use="roc")
# m.pp.ck.ust2=FindMarkers(data_ck_ust,group.by="treatment",ident.1="post", test.use="roc")
# m.pp.ck.ved2=FindMarkers(data_ck_ved,group.by="treatment",ident.1="post", test.use="roc")
# m.pp.str.inf2=FindMarkers(data_str_inf,group.by="treatment",ident.1="post", test.use="roc")
# m.pp.str.ust2=FindMarkers(data_str_ust,group.by="treatment",ident.1="post", test.use="roc")
# m.pp.str.ved2=FindMarkers(data_str_ved,group.by="treatment",ident.1="post", test.use="roc")
...
plot_g1=ggvenn(list(pre_ck_inf=m.pp.ck.inf__$gene,
                   pre_ck_ved=m.pp.ck.ved__$gene,
                   pre_ck_ust=m.pp.ck.ust__$gene))
plot_g2=ggvenn(list(post_ck_inf=m.pp.ck.inf_$gene,
                   post_ck_ved=m.pp.ck.ved_$gene,
                   post_ck_ust=m.pp.ck.ust_$gene))
plot_g3=ggvenn(list(pre_str_inf=m.pp.str.inf__$gene,
                   pre_str_ved=m.pp.str.ved__$gene,
                   pre_str_ust=m.pp.str.ust__$gene))
plot_g4=ggvenn(list(post_str_inf=m.pp.str.inf_$gene,
                   post_str_ved=m.pp.str.ved_$gene,
                   post_str_ust=m.pp.str.ust_$gene))
...
# Response
m.r=FindMarkers(data_seurat,group.by="Endo remission",ident.1="1")
m.r.ck=FindMarkers(data_ck,group.by="Endo remission",ident.1="1")
m.r.str=FindMarkers(data_str,group.by="Endo remission",ident.1="1")
m.r.ck.inf=FindMarkers(data_ck_inf,group.by="Endo remission",ident.1="1")
m.r.ck.ust=FindMarkers(data_ck_ust,group.by="Endo remission",ident.1="1")
m.r.ck.ved=FindMarkers(data_ck_ved,group.by="Endo remission",ident.1="1")
m.r.str.inf=FindMarkers(data_str_inf,group.by="Endo remission",ident.1="1")
m.r.str.ust=FindMarkers(data_str_ust,group.by="Endo remission",ident.1="1")
m.r.str.ved=FindMarkers(data_str_ved,group.by="Endo remission",ident.1="1")
...
plot_g11=ggvenn(list(NR_ck_inf=m.r.ck.inf__$gene,
                   NR_ck_ved=m.r.ck.ved__$gene,
                   NR_ck_ust=m.r.ck.ust__$gene))
plot_g12=ggvenn(list(R_ck_inf=m.r.ck.inf_$gene,
                   R_ck_ved=m.r.ck.ved_$gene,
                   R_ck_ust=m.r.ck.ust_$gene))
plot_g13=ggvenn(list(NR_str_inf=m.r.str.inf__$gene,
                   NR_str_ved=m.r.str.ved__$gene,
                   NR_str_ust=m.r.str.ust__$gene))
plot_g14=ggvenn(list(R_str_inf=m.r.str.inf_$gene,
                   R_str_ved=m.r.str.ved_$gene,
                   R_str_ust=m.r.str.ust_$gene))
...
# m.r.pp.ck=FindMarkers(data_ck,group.by="treatment",ident.1="post")
# m.r.pp.str=FindMarkers(data_str,group.by="treatment",ident.1="post")
m.r.pp.ck.inf=FindMarkers(data.r.ck_inf,group.by="treatment",ident.1="post")
m.r.pp.ck.ust=FindMarkers(data.r.ck_ust,group.by="treatment",ident.1="post")
m.r.pp.ck.ved=FindMarkers(data.r.ck_ved,group.by="treatment",ident.1="post")
m.r.pp.str.inf=FindMarkers(data.r.str_inf,group.by="treatment",ident.1="post")
m.r.pp.str.ust=FindMarkers(data.r.str_ust,group.by="treatment",ident.1="post")
m.r.pp.str.ved=FindMarkers(data.r.str_ved,group.by="treatment",ident.1="post")
...
# m.nr.pp=FindMarkers(data_seurat,group.by="treatment",ident.1="post")
# m.nr.pp.ck=FindMarkers(data_ck,group.by="treatment",ident.1="post")
# m.nr.pp.str=FindMarkers(data_str,group.by="treatment",ident.1="post")
m.nr.pp.ck.inf=FindMarkers(data.nr.ck_inf,group.by="treatment",ident.1="post")
m.nr.pp.ck.ust=FindMarkers(data.nr.ck_ust,group.by="treatment",ident.1="post")
m.nr.pp.ck.ved=FindMarkers(data.nr.ck_ved,group.by="treatment",ident.1="post")
m.nr.pp.str.inf=FindMarkers(data.nr.str_inf,group.by="treatment",ident.1="post")
m.nr.pp.str.ust=FindMarkers(data.nr.str_ust,group.by="treatment",ident.1="post")
m.nr.pp.str.ved=FindMarkers(data.nr.str_ved,group.by="treatment",ident.1="post")
...
# genes_ust <- m.pp.ck.ust__$gene
# genes_ved <- m.pp.ck.ved__$gene
# genes_inf <- m.pp.ck.inf_$gene
# genes_ust <- m.pp.ck.ust_$gene
# genes_ved <- m.pp.ck.ved_$gene
# genes_inf <- m.pp.str.inf__$gene
# genes_ust <- m.pp.str.ust__$gene
# genes_ved <- m.pp.str.ved__$gene
# genes_inf <- m.pp.str.inf_$gene
# genes_ust <- m.pp.str.ust_$gene
# genes_ved <- m.pp.str.ved_$gene


# genes_inf <- m.r.ck.inf__$gene
# genes_ust <- m.r.ck.ust__$gene
# genes_ved <- m.r.ck.ved__$gene
genes_inf <- m.r.ck.inf_$gene
genes_ust <- m.r.ck.ust_$gene
genes_ved <- m.r.ck.ved_$gene
# genes_inf <- m.r.str.inf__$gene
# genes_ust <- m.r.str.ust__$gene
# genes_ved <- m.r.str.ved__$gene
# genes_inf <- m.r.str.inf_$gene
# genes_ust <- m.r.str.ust_$gene
# genes_ved <- m.r.str.ved_$gene

# 2. 배경 유전자 정의 (아래 두 가지 방법 중 하나 선택)
# 방법 A: 각 데이터의 모든 유전자의 합집합 (추천)
bg_genes <- union(rownames(data_ck_inf), 
                  union(rownames(data_ck_ust), rownames(data_ck_ved)))
...
# 1. 중복 유전자 수 계산
overlap_inf_ved <- length(intersect(genes_inf, genes_ved))
overlap_inf_ust <- length(intersect(genes_inf, genes_ust))
overlap_ved_ust <- length(intersect(genes_ved, genes_ust))
...
#'
#' @param ... A variable number of character vectors, each representing a set (e.g., gene list). It's best to provide them as named arguments (e.g., inf = genes_inf, ust = genes_ust).
...
bg_genes <- union(rownames(data_str_inf),
                  union(rownames(data_str_ust), rownames(data_str_ved)))
...
# metadata에는 emrid, treatment, ck, drug 열이 포함되어야 함
# 특정 그룹으로 미리 필터링했다고 가정 (예: Drug A, ck==TRUE)
# filtered_counts_matrix, filtered_metadata

# 1. DESeqDataSet 객체 생성
dds <- DESeqDataSetFromMatrix(
  countData = filtered_counts_matrix,
  colData = filtered_metadata,
  design = ~ emrid + treatment # 환자 효과 보정 + treatment 효과 분석
)
...
# 4. 결과 추출
res_deseq_paired <- results(dds, name = "treatment_post_vs_pre")
head(res_deseq_paired)
```
...
myhm_genesets2(subset(data_seurat,drug=="Infliximab"), group="heal", assay = "RNA", gene_sets = module_list)
myhm_genesets2(subset(data_seurat,drug!="Infliximab"), group="heal", assay = "RNA", gene_sets = module_list)
myhm_genesets2(subset(data_ck,drug=="Infliximab"), group="heal", assay = "RNA", gene_sets = module_list)
myhm_genesets2(subset(data_ck,drug!="Infliximab"), group="heal", assay = "RNA", gene_sets = module_list)
myhm_genesets2(subset(data_str,drug=="Infliximab"), group="heal", assay = "RNA", gene_sets = module_list)
myhm_genesets2(subset(data_str,drug!="Infliximab"), group="heal", assay = "RNA", gene_sets = module_list)

# VlnPlot도 다 나눠서 해봐야....


data_seurat2$drug.r=paste0(data_seurat2$drug,"_" ,data_seurat2$`Endo remission`)

myhm_genesets3(subset(data_seurat2,ck==TRUE), group="drug.x", assay = "RNA", gene_sets = module_total)
myhm_genesets3(subset(data_seurat2,ck==TRUE), group="drug.r", assay = "RNA", gene_sets = module_total)
